...
void test() {
    ...
    bool v2 = sylar::Address::Lookup(addrs, "www.baidu.com", AF_INET);
    bool v3 = sylar::Address::Lookup(addrs, "www.sylar.top", AF_INET);
    SYLAR_LOG_INFO(g_logger) << "end";
    if(!v) {
        SYLAR_LOG_ERROR(g_logger) << "v1  lookup fail";
    }
    if(!v2) {
        SYLAR_LOG_ERROR(g_logger) << "v2  lookup fail";
    }
    if(!v3) {
        SYLAR_LOG_ERROR(g_logger) << "v3  lookup fail";
    }
