...
sylar::ConfigVar<std::unordered_set<int> >::ptr g_int_uset_value_config = 
    sylar::Config::Lookup("system.int_uset", std::unordered_set<int>{77,88,99}, "system int uset");

sylar::ConfigVar<std::map<std::string, int> >::ptr g_int_map_value_config = 
    sylar::Config::Lookup("system.str_int_map", std::map<std::string, int>{ {"k1", 2}}, "system str int map");

sylar::ConfigVar<std::unordered_map<std::string, int> >::ptr g_int_umap_value_config = 
    sylar::Config::Lookup("system.str_int_umap", std::unordered_map<std::string, int>{ {"uk5", 5}}, "system str int umap");
...
void test_yaml(){
    YAML::Node root = YAML::LoadFile("/home/henry/workspace/henry-sylar/bin/conf/test.yml");
    print_yaml(root, 0);
...
void test_log(){
    ...
    std::cout<< sylar::LoggerMgr::GetInstance()->toYamlString() << std::endl;
                                   ///home/henry/workspace/henry-sylar/bin/conf
    YAML::Node root = YAML::LoadFile("/home/henry/workspace/henry-sylar/bin/conf/test.yml");
    sylar::Config::LoadFromYaml(root);
    std::cout<<"======================"<<std::endl;
    std::cout<< sylar::LoggerMgr::GetInstance()->toYamlString() << std::endl;
