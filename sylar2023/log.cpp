            ...
            if(n["appenders"].IsDefined()){
                ...
                for(size_t x=0; x<n["appenders"].size();++x){
                    ...
                    if(type == "FileLogAppender"){
                        lad.type = 1;
                        if(!a["file"].IsDefined()){
                            std::cout<<" log config error : fileappender file is invalid "<<a << std::endl;
                            continue;
                        }
...
struct LogIniter{
    LogIniter(){
        g_log_defines->addListener(0xF1E234, [](const std::set<LogDefine>& old_value,
